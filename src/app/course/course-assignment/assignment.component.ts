...
export class AssignmentComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this.route.params.subscribe((p) => {
			...
			this.assignmentService.getAssignment(cId, p.aId, (a) =>
				this.setAssignment(cId, a)
			);
		});
	}
	...
	private setAssignment(cId: number, assignment: Assignment): void {
		...
		this.assignmentBody = this.sanitizer.bypassSecurityTrustHtml(
			assignment.description
		);
		this.isAssignmentGraded =
			assignment.submission_types.join() !== "not_graded";
		...
		if (assignment.url && assignment.external_tool_tag_attributes) {
			this.courseService.getExternalSessionlessLaunch(
				cId,
				assignment.id,
				"assessment",
				(res) => {
