...
export abstract class APIBaseService {
	...
	async xfetch<T>(
		...
		const cacheId = options?.params
			? `${endpoint}.${options.params.toString()}`
			: endpoint;
		...
		if (options?.method ?? "GET" === "GET") {
			...
			if (cached && cached.value) {
				...
				if (cached.link)
					pageInfo = this.buildPaginationInfo<T>(
						cached.link,
						callback,
						options
					);
				...
				const reducedNetwork = this.configService.getVal<boolean>(
					"networking",
					"stop_calls_if_cached"
				);
				const cacheMaxLongDuration =
					options?.cacheLong ?? cacheMaxShortDuration + 20000;
				if (reducedNetwork && cacheElapsedTime < cacheMaxLongDuration) {
					this.notifService.notify(
						"Low Bandwith Mode: You might be seeing stale data.",
						1
					);
					return;
				}
		...
		const offlineMode = this.configService.getVal<boolean>(
			"networking",
			"offline_mode"
		);
		if (offlineMode) {
			this.notifService.notify(
				"Your data is frozen. Network requests to get or set data are paused.",
				0
			);
			return;
		}
		...
		if (!domain) {
			this.notifService.notify(
				"There is a caravel.domain configuration error.",
				0
			);
			return;
		}
		...
		await fetch(`https://${proxy}/${url}`, {
			...
			.then(async (response) => {
				...
				if (callElapsed > 2000)
					console.warn(
						`[WARN] [${this.scope}, ${endpoint}] Encountered slow network: ${callElapsed}ms`
					);
			})
			.catch((ex) => {
				...
				if (cached?.value)
					this.notifService.notify(
						"You are seeing stale data. It may not be up to date.",
						1
					);
				else this.notifService.notify("Failed to get data (network error).", 0);
