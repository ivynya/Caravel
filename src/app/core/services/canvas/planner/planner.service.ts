...
export class PlannerService extends APIBaseService {
  ...
  setPlannerOverride(type: string, id: number, isComplete: boolean,
                     callback: ResultHandler<PlannerOverride>): void {
    const qp = {
      plannable_type: type,
      plannable_id: `${id}`,
    ...
    this.xfetch<PlannerOverride>(
      `overrides`, callback,
      { method: "POST", params: new URLSearchParams(qp) });
  }

  // Updates an existing PlannerOverride with data
  updatePlannerOverride(pId: number, isComplete: boolean,
                        callback: ResultHandler<PlannerOverride>): void {
    const qp = {
      marked_complete: `${isComplete}`,
      dismissed: "false"
    ...
    this.xfetch<PlannerOverride>(
      `overrides/${pId}`, callback,
      { method: "PUT", params: new URLSearchParams(qp) });
  }
}
