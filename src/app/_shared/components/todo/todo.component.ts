...
import {
	...
	CourseService,
	PlannerService,
} from "../../../_core/services/canvas";
import { Course, PlannerItem } from "../../../_core/schemas";
	...
	ngOnInit(): void {
		...
		if (this.item.plannable_type === "assignment")
			this.assignmentService.getAssignment(
				this.item.course_id,
				this.item.plannable.id,
				(a) => {
					this.isLocked = a.locked_for_user;
				}
			);
		else if (this.item.plannable_type === "wiki_page")
			this.courseService.getCoursePage(
				this.item.course_id,
				this.item.plannable.url,
				(p) => {
