...
import { RoundDatePipe } from "../../../pipes";
...
export class CourseService extends APIBaseService {
	...
	listExternalTools(
		cId: number,
		callback: (data: ExternalTool[]) => void
	): void {
	...
	getExternalSessionlessLaunch(
		cId: number,
		aId: number,
		type: string,
		callback: (data: SessionlessLaunch) => void
	): void {
		const qp = {
		...
		this.xfetch<SessionlessLaunch>(
			`${cId}/external_tools/sessionless_launch`,
			(res) => {
				callback(res.data);
			},
			{ cacheShort: 0, cacheLong: 0, params: new URLSearchParams(qp) }
		).catch((ex) => console.error(ex));
	}

	listCourseRecentSubmissions(
		cId: number,
		callback: (data: Submission[]) => void
	): void {
		const loadTo = this.roundDate.transform(
			new Date(new Date().getTime() - 86400000 * 31)
		);
		const qp = {
	...
	listModules(cId: number, callback: ResultHandler<Module[]>): void {
		this.xfetch<Module[]>(`${cId}/modules`, callback).catch((ex) =>
			console.error(ex)
		);
	}
	...
	getModuleItems(
		cId: number,
		mId: number,
		callback: ResultHandler<ModuleItem[]>
	): void {
		this.xfetch<ModuleItem[]>(`${cId}/modules/${mId}/items`, callback).catch(
			(ex) => console.error(ex)
		);
	}
}
